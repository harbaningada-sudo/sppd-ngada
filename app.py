...
<style>
    ...
    /* KHUSUS HALAMAN BELAKANG TURUN LAGI AGAR PAS CETAK */
    .kertas-belakang { margin-top: 55mm !important; }
    ...
    .tabel-border td { border: 1pt solid black !important; padding: 4px 6px !important; vertical-align: top; color: black !important; font-size: 9.2pt; }
    
    /* TABEL KHUSUS VISUM */
    .visum-table { width: 100%; border: none !important; border-collapse: collapse; margin: 0 !important; }
    .visum-table td { border: none !important; padding: 0 !important; font-size: 9.2pt; line-height: 1.2; color: black !important; vertical-align: top; }
    ...
    @media print {
        ...
        .kertas-belakang { margin-top: 50mm !important; } /* Turun lebih banyak saat diprint */
...
with st.sidebar:
    ...
    with st.expander("📄 DATA SURAT", expanded=True):
        ...
        maksud = st.text_area("Maksud Perjalanan", "Dalam Rangka Mendampingi Kunjungan...")
    ...
    with st.expander("👤 DATA PEGAWAI", expanded=True):
        if 'jml_peg' not in st.session_state: st.session_state.jml_peg = 1
        col1, col2 = st.columns(2)
        with col1:
            if st.button("➕ Tambah"): st.session_state.jml_peg += 1
        with col2:
            if st.button("➖ Hapus") and st.session_state.jml_peg > 1: st.session_state.jml_peg -= 1
        
        daftar = []
        for i in range(st.session_state.jml_peg):
            st.markdown(f"**Pegawai ke-{i+1}**")
...
ttd_global = f'''<div style="margin-left:55%; margin-top:15px; line-height:1.2; color:black; font-size:10pt;"><table class="visum-table"><tr><td width="40%">Ditetapkan di</td><td width="5%">:</td><td>Bajawa</td></tr><tr><td>Pada Tanggal</td><td>:</td><td>{tgl_berangkat}</td></tr></table><br><b>An. BUPATI NGADA</b><br>{jab_ttd},<br><br><br><br><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''

html_out = '<div class="main-container">'

# 1. CETAK SPT (1.5 SPACING ISI)
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        ...
        def row_v(num, label, val, date_v, show_num=True):
            n = f'<td width="10%">{num}</td>' if show_num else ""
            return f'''<table class="visum-table"><tr>{n}<td width="35%">{label}</td><td width="3%">:</td><td>{val}</td></tr><tr>{"<td></td>" if show_num else ""}<td>Pada Tanggal</td><td>:</td><td>{date_v}</td></tr></table>'''

        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{row_v("I.", "Berangkat dari", "Bajawa", tgl_berangkat)}<table class="visum-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="3%">:</td><td>{tujuan}</td></tr></table>{ttd_v}</td></tr>
            <tr style="height: 155px;"><td>{row_v("II.", "Tiba di", tujuan, tgl_berangkat)}</td><td style="padding:5px;">{row_v("", "Berangkat dari", tujuan, tgl_kembali, False)}<table class="visum-table"><tr><td width="35%">Ke</td><td width="3%">:</td><td>Bajawa</td></tr></table></td></tr>
            <tr style="height: 150px;"><td>{row_v("III.", "Tiba di", "", "")}</td><td style="padding:5px;">{row_v("", "Berangkat dari", "", "", False)}</td></tr>
            <tr style="height: 150px;"><td>{row_v("IV.", "Tiba di", "", "")}</td><td style="padding:5px;">{row_v("", "Berangkat dari", "", "", False)}</td></tr>
            <tr style="height: 180px;"><td>{row_v("V.", "Tiba Kembali", "Bajawa", tgl_kembali)}</td><td style="padding:5px;"><p style="font-style:italic; font-size:8.8pt; line-height:1.2; margin-top:2px;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya dan semata-mata untuk kepentingan jabatan</p>{ttd_v}</td></tr>
        </table><div style="border:1pt solid black; border-top:none; padding:5px; font-size:10pt;"><b>VI. Catatan Lain-lain</b></div>
        <div style="border:1pt solid black; border-top:none; padding:8px; font-size:8.2pt; text-align:justify; color:black; line-height:1.2;">
            <b>VII. Perhatian :</b><br>Pejabat yang menerbitkan SPD, pegawai yang melakukan perjalanan dinas, para pejabat yang mengesahkan tanggal berangkat/tiba, serta Bendahara Pengeluaran bertanggung jawab berdasarkan peraturan-peraturan Keuangan Negara apabila negara menderita rugi akibat kesalahan, kelalaian dan kealpaannya.
        </div></div>'''

# 3. REGISTER
if "Register" in opsi_cetak:
    rows_reg = "".join([f"<tr><td class='text-center'>{i+1}</td><td>{p['nama']}</td><td>{no_spt}</td><td>{p['spd']}</td><td>{tgl_berangkat}</td><td>{tujuan}</td></tr>" for i, p in enumerate(daftar)])
    html_out += f'''<div class="kertas"><h3 class="text-center text-bold">REGISTER SURAT PERJALANAN DINAS</h3><br><table class="tabel-border"><thead><tr style="background:#eee;"><th width="5%">No</th><th>Nama Pegawai</th><th>No SPT</th><th>No SPD</th><th>Tgl Berangkat</th><th>Tujuan</th></tr></thead><tbody>{rows_reg}</tbody></table></div>'''
