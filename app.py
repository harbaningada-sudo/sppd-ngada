    ...
    if tab_menu == "Input & Cetak":
        ...
        if jenis_spd == "Luar Daerah":
            with st.expander("🏢 TUJUAN LUAR DAERAH"):
                ...
                gol_tujuan = st.text_input("Gol Tujuan", "Pembina Utama Muda, IV/c")
                nip_tujuan = st.text_input("NIP Tujuan", "19XXXXXXXXXXXXXX")

        st.subheader("🖋️ TANDA TANGAN")
        ttd_label_spt = st.selectbox("Label TTD SPT", ["BUPATI NGADA", "WAKIL BUPATI NGADA", "An. BUPATI NGADA"])
...
# --- TEMPLATE KOMPONEN ---
def get_ttd(space, label_override=None): 
    lbl = label_override if label_override else "An. BUPATI NGADA"
    return f'''<div style="margin-left:55%; margin-top:10px; line-height:1.0; text-align:center;"><b>{lbl}</b><br>{jab_ttd if lbl == "An. BUPATI NGADA" else ""}{f"<br>Ub. {ub}," if (lbl == "An. BUPATI NGADA" and ub) else ""}<div style="height:{space}px;"></div><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''
    ...
    for p in daftar:
        if "SPD Depan" in opsi_cetak:
            ...
            html_out += f'''<div class="kertas">{kop_pemda}<div style="margin-left:60%; line-height:1.0;"><table class="visum-table"><tr><td width="40%">Lembar ke</td><td width="5%">:</td><td>{p["lembar"]}</td></tr><tr><td>Kode No</td><td>:</td><td>{kode_spd}</td></tr><tr><td>Nomor</td><td>:</td><td>{p["spd"]}</td></tr></table></div><div class="judul-rapat" style="margin-top:5px;"><h3 class="underline">SURAT PERJALANAN DINAS</h3><h3 class="text-bold">(SPD)</h3></div><table class="tabel-border">
                ...
                <tr><td class="col-no">7.</td><td>Lamanya Perjalanan Dinas</td><td colspan="3">{lama}</td></tr>
            </table>{get_ttd(75, "An. BUPATI NGADA")}</div>'''

    # 3. SPD BELAKANG
    if "SPD Belakang" in opsi_cetak:
        ttd_asal = get_ttd(65, "An. BUPATI NGADA")
        ttd_tujuan = f'''<div style="text-align:center; line-height:1.0; font-size:10pt;"><br><b>Mengesahkan</b><br>{instansi_tujuan}<div style="height:60px;"></div><b><u>{pjb_tujuan}</u></b><br>{gol_tujuan}<br>NIP. {nip_tujuan}</div>''' if jenis_spd == "Luar Daerah" else ""
        ...
        html_out += f'''<div class="kertas"><table class="tabel-border" style="height:88%;">
            ...
            <tr style="height: 190px;"><td>{rv("II.", "Tiba di", tujuan, tgl_bkt)}</td><td style="padding:10px;">{rv("", "Berangkat dari", tujuan, tgl_kbl, False)}<br>{ttd_tujuan}</td></tr>
            <tr style="height: 220px;"><td>{rv("V.", "Tiba Kembali", "Bajawa", tgl_kbl)}</td><td style="padding:10px;"><p style="font-style:italic; font-size:9.2pt; line-height:1.0;">Telah diperiksa...</p>{ttd_asal}</td></tr>
        </table></div>'''
