...
# CSS UNTUK UKURAN LEGAL & FIT SHEET
st.markdown("""
<style>
    ...
    .kertas { 
        ...
        font-size: 11pt; /* Ukuran font standar diperbesar sedikit */
    }
    ...
    @media print {
        ...
        .kertas { 
            ...
            transform: scale(1); /* Memastikan tidak mengecil */
        }
        .kertas-belakang { margin-top: 80mm !important; } 
        @page { 
            size: legal; 
            margin: 0; 
        }
...
with st.sidebar:
    ...
    with st.expander("👤 DATA PEGAWAI", expanded=True):
        ...
        c1, c2 = st.columns(2)
        if c1.button("➕ Tambah"): st.session_state.jml_peg += 1
        if c2.button("➖ Hapus") and st.session_state.jml_peg > 1: st.session_state.jml_peg -= 1
    ...
    if st.button("🖨️ PROSES CETAK"):
        st.components.v1.html("<script>setTimeout(function(){ window.parent.print(); }, 1500);</script>", height=0)
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        ttd_v = f'<div style="text-align:center; line-height:1.1; font-size:10pt;"><br><b>An. BUPATI NGADA</b><br>{jab_ttd},<br>{f"Ub. {ub}," if ub else ""}<br><br><br><br><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'
        ...
        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{rv("I.", "Berangkat dari", "Bajawa", tgl_bkt)}<table class="visum-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="4%">:</td><td>{tujuan}</td></tr></table>{ttd_v}</td></tr>
            <tr style="height: 155px;"><td>{rv("II.", "Tiba di", tujuan, tgl_bkt)}</td><td style="padding:5px;">{rv("", "Berangkat dari", tujuan, tgl_kbl, False)}<table class="visum-table"><tr><td width="35%">Ke</td><td width="4%">:</td><td>Bajawa</td></tr></table></td></tr>
