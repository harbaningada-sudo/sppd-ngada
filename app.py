...
<style>
    ...
    /* KHUSUS HALAMAN BELAKANG TURUN AGAR TIDAK TERPOTONG */
    .kertas-belakang { margin-top: 25mm !important; }
    ...
    .tabel-border td { border: 1pt solid black !important; padding: 2px 4px !important; vertical-align: top; color: black !important; font-size: 9pt; }
    
    /* TABEL KHUSUS VISUM */
    .visum-table { width: 100%; border: none !important; border-collapse: collapse; margin: 0 !important; }
    .visum-table td { border: none !important; padding: 0 !important; font-size: 9pt; line-height: 1.1; color: black !important; vertical-align: top; }
...
with st.sidebar:
    ...
    opsi_cetak = st.multiselect("Pilih Dokumen Cetak", ["SPT", "SPD Depan", "SPD Belakang", "Register"], default=["SPT", "SPD Depan", "SPD Belakang"])
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        ...
        def row_v(num, label, val, date_v, show_num=True):
            n = f'<td width="8%">{num}</td>' if show_num else ""
            return f'''<table class="visum-table"><tr>{n}<td width="32%">{label}</td><td width="3%">:</td><td>{val}</td></tr><tr>{"<td></td>" if show_num else ""}<td>Pada Tanggal</td><td>:</td><td>{date_v}</td></tr></table>'''

        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{row_v("I.", "Berangkat dari", "Bajawa", tgl_berangkat)}<table class="visum-table"><tr><td width="8%"></td><td width="32%">Ke</td><td width="3%">:</td><td>{tujuan}</td></tr></table>{ttd_v}</td></tr>
            <tr style="height: 155px;"><td>{row_v("II.", "Tiba di", tujuan, tgl_berangkat)}</td><td>{row_v("", "Berangkat dari", tujuan, tgl_kembali, False)}<table class="visum-table"><tr><td width="32%">Ke</td><td width="3%">:</td><td>Bajawa</td></tr></table></td></tr>
            <tr style="height: 150px;"><td>{row_v("III.", "Tiba di", "", "")}</td><td>{row_v("", "Berangkat dari", "", "", False)}</td></tr>
            <tr style="height: 150px;"><td>{row_v("IV.", "Tiba di", "", "")}</td><td>{row_v("", "Berangkat dari", "", "", False)}</td></tr>
...
if "Register" in opsi_cetak:
    rows_reg = "".join([f"<tr><td class='text-center'>{i+1}</td><td>{p['nama']}</td><td>{no_spt}</td><td>{p['spd']}</td><td>{tgl_berangkat}</td><td>{tujuan}</td><td>{maksud}</td></tr>" for i, p in enumerate(daftar)])
    html_out += f'''<div class="kertas"><h3 class="text-center text-bold">REGISTER SURAT PERJALANAN DINAS</h3><br><table class="tabel-border"><thead><tr style="background:#eee;"><th width="5%">No</th><th width="20%">Nama Pegawai</th><th width="15%">No SPT</th><th width="15%">No SPD</th><th width="15%">Tgl Berangkat</th><th width="15%">Tujuan</th><th>Maksud Perjalanan</th></tr></thead><tbody>{rows_reg}</tbody></table></div>'''
