...
st.components.v1.html(surat_html, height=2500, scrolling=True)import streamlit as st
from datetime import datetime
...
with st.sidebar:
    ...
    with st.expander("📄 DATA SURAT", expanded=True):
        ...
        maksud = st.text_area("Maksud Perjalanan", "Monitoring dan Pendataan Pemilik Tambang Pasir...")
        tujuan = st.text_input("Tempat Tujuan", "Kecamatan Bajawa")
    ...
    with st.expander("🕒 WAKTU & ANGGARAN", expanded=False):
        tgl_p = st.date_input("Tanggal Berangkat", datetime(2026, 2, 11))
        tgl_k = st.date_input("Tanggal Kembali", datetime(2026, 2, 11))
        lama = st.text_input("Lama Hari", "1 (Satu)")
    ...
    with st.expander("🖋️ PENANDATANGAN", expanded=False):
        tgl_cetak = st.date_input("Tanggal Cetak", datetime(2026, 2, 11))
        ttd_nama = st.text_input("Nama TTD", "Alfian, S.Sos")
        ttd_nip = st.text_input("NIP TTD", "19710913 199403 1 008")
        ttd_gol = st.text_input("Gol TTD", "Pembina Utama Muda - IV/c")
...
# FORMAT TANGGAL
bulan_list = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"]
tgl_indo = f"{tgl_cetak.day} {bulan_list[tgl_cetak.month-1]} {tgl_cetak.year}"

# 3. TEMPLATE HTML UNTUK SPT & SPD
surat_html = f"""
<style>
    .wrap {{ background-color: #525659; padding: 20px; display: flex; flex-direction: column; align-items: center; gap: 20px; }}
    .kertas {{ background-color: white; width: 210mm; min-height: 297mm; padding: 15mm 20mm 20mm 25mm; color: black; font-family: Arial, sans-serif; font-size: 10.5pt; box-shadow: 0 0 15px rgba(0,0,0,0.5); box-sizing: border-box; page-break-after: always; }}
    .kop {{ text-align: center; border-bottom: 3px solid black; padding-bottom: 2px; margin-bottom: 15px; line-height: 1.2; }}
    .tabel-polos {{ width: 100%; border-collapse: collapse; }}
    .tabel-polos td {{ border: none; padding: 2px; vertical-align: top; }}
    .tabel-border {{ width: 100%; border-collapse: collapse; border: 1.2px solid black; }}
    .tabel-border td {{ border: 1.2px solid black; padding: 5px 8px; vertical-align: top; }}
    .text-center {{ text-align: center; }}
    .text-bold {{ font-weight: bold; }}
    .text-underline {{ text-decoration: underline; }}
</style>
...
<div class="wrap">
    <div class="kertas">
        ...
        <p class="text-center" style="margin-top:0;">NOMOR : 094/Prokopim/557/02/2026</p>
        <br>
        <table class="tabel-polos">
            <tr><td width="15%">Dasar</td><td width="2%">:</td><td>DPA Bagian Perekonomian dan SDA Setda Ngada Tahun Anggaran 2026</td></tr>
        </table>
        ...
        <div style="margin-left:55%; margin-top:40px;">
            <p style="margin:0;">Ditetapkan di : Bajawa</p>
            <p style="margin:0;">Pada Tanggal : {tgl_indo}</p>
            <br><p class="text-bold" style="margin:0;">An. BUPATI NGADA</p>
            <p style="margin:0;">Sekretaris Daerah,</p>
            <br><br><br><br>
            <p class="text-bold text-underline" style="margin:0;">{ttd_nama}</p>
            <p style="margin:0;">{ttd_gol}</p>
            <p style="margin:0;">NIP. {ttd_nip}</p>
    ...
    <div class="kertas">
        ...
        <table class="tabel-border">
            ...
            <tr><td>7.</td><td>a. Lamanya Perjalanan Dinas<br>b. Tanggal Berangkat<br>c. Tanggal Harus Kembali</td><td>{lama}<br>{tgl_p.day} {bulan_list[tgl_p.month-1]} {tgl_p.year}<br>{tgl_k.day} {bulan_list[tgl_k.month-1]} {tgl_k.year}</td></tr>
            <tr><td>8.</td><td>Pengikut: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Nama</td><td class="text-center">Tanggal Lahir / Keterangan</td></tr>
            <tr><td height="30px"></td><td>1.</td><td></td></tr>
        ...
        <div style="margin-left:55%; margin-top:20px;">
            ...
            <br><p class="text-bold" style="margin:0;">An. BUPATI NGADA</p>
            <p style="margin:0;">Sekretaris Daerah,</p>
            <p style="margin:0;">Ub. Asisten Pemerintahan dan Kesra</p>
            <br><br><br><br>
            <p class="text-bold text-underline" style="margin:0;">{ttd_nama}</p>
            <p style="margin:0;">{ttd_gol}</p>
            <p style="margin:0;">NIP. {ttd_nip}</p>
...
st.components.v1.html(surat_html, height=2000, scrolling=True)
