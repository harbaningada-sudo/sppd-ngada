...
<style>
    ...
    /* TABEL POLOS UNTUK SEJAJARKAN TITIK DUA */
    .inner-table { width: 100%; border: none !important; border-collapse: collapse; margin: 0 !important; }
    .inner-table td { border: none !important; padding: 0 !important; font-size: 9.2pt; color: black !important; }
...
for p in daftar:
    ...
    # 3. SPD BELAKANG (VISUM - TITIK DUA SEJAJAR & ISIAN NAIK 1 BARIS)
    ttd_v = f'<div style="text-align:center; line-height:1.2; font-size:9.5pt;"><br><b>An. BUPATI NGADA</b><br>{jab_ttd},<br><br><br><br><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'
    
    def visum_item(num, label, value, date_val="", is_left=True):
        num_str = f'<td width="10%">{num}</td>' if is_left else ""
        return f'''<table class="inner-table">
            <tr>{num_str}<td width="40%">{label}</td><td width="5%">:</td><td>{value}</td></tr>
            <tr>{"<td></td>" if is_left else ""}<td>Pada Tanggal</td><td>:</td><td>{date_val}</td></tr>
        </table>'''

    html_out += f'''<div class="kertas"><table class="tabel-border">
        <tr style="height: 180px;"><td width="50%"></td><td style="padding:8px;">{visum_item("I.", "Berangkat dari", "Bajawa", tgl_str(datetime.now()))}
            <table class="inner-table"><tr><td width="10%"></td><td width="40%">Ke</td><td width="5%">:</td><td>{tujuan}</td></tr></table>{ttd_v}</td></tr>
        <tr style="height: 160px;"><td>{visum_item("II.", "Tiba di", tujuan, tgl_str(datetime.now()))}</td>
            <td>&nbsp;&nbsp;&nbsp; {visum_item("", "Berangkat dari", tujuan, tgl_str(datetime.now()), False)}
            <table class="inner-table"><tr><td width="40%">Ke</td><td width="5%">:</td><td>Bajawa</td></tr></table></td></tr>
        <tr style="height: 155px;"><td>{visum_item("III.", "Tiba di", "", "")}</td>
            <td>&nbsp;&nbsp;&nbsp; {visum_item("", "Berangkat dari", "", "", False)}</td></tr>
        <tr style="height: 155px;"><td>{visum_item("IV.", "Tiba di", "", "")}</td>
            <td>&nbsp;&nbsp;&nbsp; {visum_item("", "Berangkat dari", "", "", False)}</td></tr>
        <tr style="height: 180px;"><td>{visum_item("V.", "Tiba Kembali", "Bajawa", tgl_str(datetime.now()))}</td>
            <td><p style="font-style:italic; font-size:9pt; line-height:1.2; margin-top:2px;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya dan semata-mata untuk kepentingan jabatan</p>{ttd_v}</td></tr>
    </table><div style="border:1pt solid black; border-top:none; padding:8px; font-size:10.5pt;"><b>VI. Catatan Lain-lain</b></div>
