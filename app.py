...
with st.sidebar:
    ...
    if jenis_spd == "Luar Daerah":
        with st.expander("🏢 TUJUAN LUAR DAERAH"):
            instansi_tujuan = st.text_input("Instansi Tujuan", "Kantor BBKSDA NTT")
            pjb_tujuan = st.text_input("Pejabat Pengesah", "Nama Pejabat Tujuan")
            gol_tujuan = st.text_input("Gol Tujuan", "Pembina Utama Madya, IV/d")
    ...
    nip_ttd = st.text_input("NIP Pejabat", "19650101 198603 1 045")

    if st.button("🖨️ PROSES CETAK"):
        st.components.v1.html("<script>setTimeout(function(){ window.parent.print(); }, 1000);</script>", height=0)

# --- TEMPLATE KOMPONEN ---
def get_ttd_statis(nama, gol, nip, label="An. BUPATI NGADA", space=70):
    return f'''<div style="text-align:center; line-height:1.2; font-size:10pt;"><b>{label}</b><br><div style="height:{space}px;"></div><b><u>{nama}</u></b><br>{gol}<br>NIP. {nip}</div>'''

html_out = '<div class="main-container">'

# 1. SPT (Logic Garuda Khusus Luar Daerah)
if "SPT" in opsi_cetak:
    ...
    p_rows = "".join([f"<tr><td width='12%'>Kepada</td><td width='5%'>:</td><td width='5%'>{i+1}.</td><td width='18%'>Nama</td><td width='5%'>:</td><td><b>{p['nama']}</b></td></tr><tr><td></td><td></td><td></td><td>NIP</td><td>:</td><td>{p['nip']}</td></tr>" for i, p in enumerate(daftar)])
    html_out += f'''<div class="kertas">{kop_spt}
        ...
        <div class="isi-surat-spt">
            <table class="visum-table"><tr><td width="12%">Dasar</td><td width="5%">:</td><td>DPA 2026</td></tr></table>
        ...
        <div style="margin-left:55%; margin-top:30px;">
            ...
            {get_ttd_statis(pjb, gol_pjb, nip_ttd, "WAKIL BUPATI NGADA", 80)}
        </div></div>'''

# 2. SPD DEPAN (Standard Kop)
for p in daftar:
    if "SPD Depan" in opsi_cetak:
        ...
        html_out += f'''<div class="kertas">{kop_p}<div style="margin-left:60%; line-height:1.0;">Kode No: 094<br>Nomor: {p['spd']}</div><div class="judul-rapat"><h3>SURAT PERJALANAN DINAS</h3></div>
            ...
            </table><div style="margin-left:55%; margin-top:20px;">{get_ttd_statis(pjb, gol_pjb, nip_ttd, "An. BUPATI NGADA", 65)}</div></div>'''

# 3. SPD BELAKANG (Logic Luar Daerah: II Kiri Kosong)
if "SPD Belakang" in opsi_cetak:
    ...
    html_out += f'''<div class="kertas"><table class="tabel-border" style="height:88%;">
        ...
        <tr style="height: 190px;"><td>{rv("II.", "Tiba di", tujuan, tgl_bkt)}<br></td><td style="padding:10px;">{rv("", "Berangkat dari", tujuan, tgl_kbl)}<br>{ttd_tujuan}</td></tr>
