...
import logo  # Memanggil logo.py dari repository kamu
...
<style>
    ...
    /* HALAMAN BELAKANG DITURUNKAN LAGI */
    ...
    .tabel-border td { border: 1pt solid black !important; padding: 3px 5px !important; vertical-align: top; color: black !important; font-size: 9pt; }
    
    /* TABEL VISUM SEJAJAR */
    .visum-table { width: 100%; border: none !important; border-collapse: collapse; margin: 0 !important; }
    .visum-table td { border: none !important; padding: 0 !important; font-size: 9pt; line-height: 1.2; color: black !important; vertical-align: top; }
...
with st.sidebar:
    ...
    opsi_cetak = st.multiselect("Opsi Cetak", ["SPT", "SPD Depan", "SPD Belakang", "Register"], default=["SPT", "SPD Depan", "SPD Belakang"])
    ...
    with st.expander("👤 DATA PEGAWAI", expanded=True):
        ...
        c1, c2 = st.columns(2)
        if c1.button("➕ Tambah"): st.session_state.count += 1
        if c2.button("➖ Hapus") and st.session_state.count > 1: st.session_state.count -= 1
...
ttd_global = f'''<div style="margin-left:55%; margin-top:15px; line-height:1.2; color:black; font-size:10pt;"><table class="visum-table"><tr><td width="40%">Ditetapkan di</td><td width="5%">:</td><td>Bajawa</td></tr><tr><td>Pada Tanggal</td><td>:</td><td>{tgl_bkt}</td></tr></table><br><b>An. BUPATI NGADA</b><br>{jab_ttd},<br>{f"Ub. {ub}," if ub else ""}<br><br><br><br><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        ...
        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{rv("I.", "Berangkat dari", "Bajawa", tgl_bkt)}<table class="visum-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="3%">:</td><td>{tujuan}</td></tr></table>{ttd_global}</td></tr>
            ...
            <tr style="height: 180px;"><td>{rv("V.", "Tiba Kembali", "Bajawa", tgl_kbl)}</td><td style="padding:5px;"><p style="font-style:italic; font-size:8.8pt; line-height:1.2; margin-top:2px;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya dan semata-mata untuk kepentingan jabatan</p>{ttd_global}</td></tr>
...
if "Register" in opsi_cetak:
    rows = "".join([f"<tr><td class='text-center'>{i+1}</td><td>{p['nama']}</td><td>{no_spt}</td><td>{p['spd']}</td><td>{tgl_bkt}</td><td>{tujuan}</td></tr>" for i, p in enumerate(daftar)])
    html_out += f'''<div class="kertas"><h3 class="text-center text-bold">REGISTER SURAT PERJALANAN DINAS</h3><br><table class="tabel-border"><thead><tr style="background:#eee;"><th>No</th><th>Nama</th><th>No SPT</th><th>No SPD</th><th>Tgl Bkt</th><th>Tujuan</th></tr></thead><tbody>{rows}</tbody></table></div>'''
