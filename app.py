...
<style>
    ...
    .tabel-border td { border: 1pt solid black !important; padding: 4px 6px !important; vertical-align: top; color: black !important; line-height: 1.0 !important; }
    .tabel-no-border td { border: none !important; } /* Khusus Tendes */
...
with st.sidebar:
    ...
    with st.expander("📄 DATA UTAMA"):
        ...
        tujuan = st.text_input("Tujuan", "Kecamatan Bajawa")
    ...
    with st.expander("🖋️ TTD VISUM (TUJUAN)"):
        an_tujuan = st.text_input("Gelar Jabatan (a.n)", "a.n Camat Bajawa")
        jab_tujuan = st.text_input("Jabatan Teknis", "Kasie PMD")
        nama_tujuan = st.text_input("Nama Pejabat Tujuan", "Laurensius F. Ladjaradho, SE")
        nip_tujuan = st.text_input("NIP Pejabat Tujuan", "1975102 200901 1 004")
...
def render_ttd_utama(space):
    return f'''<div style="margin-left:55%; margin-top:10px; line-height:1.2;"><b>An. BUPATI NGADA</b><br>{jab_ttd},<br>{f"Ub. {ub}," if ub else ""}<div style="height:{space}px;"></div><b><u>{pjb}</u></b><br>NIP. 19710328 199203 1 011</div>'''
...
for p in daftar:
    if "SPD Depan" in opsi:
        ...
        </table>{render_ttd_utama(75)}</div>'''

# 3. SPD BELAKANG (VISUM)
def render_visum_belakang(is_tendes):
    tab_style = "tabel-no-border" if is_tendes else "tabel-border"
    ttd_tujuan = f'<div style="text-align:center;"><b>{an_tujuan}</b><br>{jab_tujuan}<div style="height:65px;"></div><b><u>{nama_tujuan}</u></b><br>NIP. {nip_tujuan}</div>'
    ttd_ngada = f'<div style="text-align:center;"><b>An. BUPATI NGADA</b><br>{jab_ttd},<br>{f"Ub. {ub}," if ub else ""}<div style="height:65px;"></div><b><u>{pjb}</u></b><br>NIP. 19710328 199203 1 011</div>'
    ...
    return f'''<div class="kertas"><table class="{tab_style}" style="height:88%;">
        <tr style="height: 195px;"><td width="50%"></td><td style="padding:10px;">{rv("I.", "Berangkat dari", "Bajawa", tgl_bkt)}<table class="visum-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="5%">:</td><td>{tujuan}</td></tr></table>{ttd_ngada}</td></tr>
        <tr style="height: 175px;"><td>{rv("II.", "Tiba di", tujuan, tgl_bkt)}<br>{ttd_tujuan}</td><td style="padding:10px;">{rv("", "Berangkat dari", tujuan, tgl_kbl, False)}</td></tr>
        <tr style="height: 175px;"><td>{rv("III.", "Tiba di", "", "")}</td><td style="padding:10px;">{rv("", "Berangkat dari", "", "", False)}</td></tr>
        <tr style="height: 175px;"><td>{rv("IV.", "Tiba di", "", "")}</td><td style="padding:10px;">{rv("", "Berangkat dari", "", "", False)}</td></tr>
        <tr style="height: 195px;"><td>{rv("V.", "Tiba Kembali", "Bajawa", tgl_kbl)}<br>{ttd_ngada}</td><td style="padding:10px;"><p style="font-style:italic; font-size:9pt; line-height:1.1;">Telah diperiksa, dengan keterangan perjalanan atas perintahnya...</p>{ttd_ngada}</td></tr>
...
if "SPD Belakang" in opsi: html_out += render_visum_belakang(False)
if "SPD Belakang (Tendes)" in opsi: html_out += render_visum_belakang(True)
