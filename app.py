...
import logo  # Memanggil file logo.py di repository
...
<style>
    ...
    .kop-table {
        ...
        border-bottom: 3.5pt solid black !important;
        margin-bottom: 5px;
    }
    ...
    .tabel-border {
        ...
        border: 1pt solid black !important; table-layout: fixed;
    }
    .tabel-border td {
...
# 3. HELPER FUNCTIONS
# ─────────────────────────────────────────────
def get_ttd(space_px, label=None, jabatan=None, ub_val=None):
    """Render blok tanda tangan."""
    lbl = label or ttd_label
    jab_line = f"{jabatan or jab_ttd}," if lbl == "An. BUPATI NGADA" else ""
    ub_line  = f"Ub. {ub_val or ub}," if (ub and lbl == "An. BUPATI NGADA") else ""
    return (
...
def render_visum(num, label, nilai, tanggal, show_num=True):
    """Render satu baris visum perjalanan."""
...
with st.sidebar:
    ...
    if tab_menu == "Input & Cetak":
        ...
        with st.expander("📄 DATA PERJALANAN", expanded=False):
            ...
            default_dasar = (
                "DPA Bagian Perekonomian dan SDA Setda Ngada 2026"
                if wilayah == "Dalam Daerah" else ""
            )
        ...
        if st.button("🖨️ PROSES CETAK & SIMPAN", use_container_width=True):
            if not daftar or not tujuan.strip():
                st.warning("Lengkapi data pegawai dan tujuan terlebih dahulu.")
            else:
                for p in daftar:
                ...
                st.success(f"{len(daftar)} data tersimpan ke register.")
...
if "SPD Belakang" in opsi_cetak:
    ...
    # Blok tiba di tujuan (beda format untuk dalam/luar daerah)
