    ...
    if st.button("🖨️ PROSES CETAK & SIMPAN"):
        for p in daftar:
            st.session_state.arsip_register.append({
                ...
                "Berangkat": tgl_bkt, "Pulang": tgl_kbl, "Lama": lama, "Ket": jenis_spd
            })
...
if tab_menu == "Input & Cetak":
    # 1. SPT (Logic Perbedaan Kop)
    if "SPT" in opsi_cetak:
        # Perbaikan: Jika Luar Daerah pakai Logo Garuda (sesuai gambar)
        if jenis_spd == "Luar Daerah":
        ...
        html_out += f'''<div class="kertas">{kop_spt}
            ...
            <div style="margin-left:55%; margin-top:20px;">
                ...
                {get_ttd_statis(pjb, gol_pjb, nip_ttd, 80)}
            </div>
        </div>'''

    # 2. SPD DEPAN (Tetap Kop Pemda)
    ...
    if "SPD Belakang" in opsi_cetak:
        ...
        ttd_tujuan = f'''<div style="text-align:center; line-height:1.2; font-size:10pt;"><br><b>Mengesahkan</b><br>{instansi_tujuan}<div style="height:65px;"></div><b><u>{pjb_tujuan}</u></b><br>{gol_tujuan}<br>NIP. {nip_tujuan}</div>''' if jenis_spd == "Luar Daerah" else ""
        ...
        html_out += f'''<div class="kertas"><table class="tabel-border" style="height:88%;">
            ...
            <tr style="height: 220px;"><td>{rv("V.", "Tiba Kembali", "Bajawa", tgl_kbl)}</td><td style="padding:10px;"><p style="font-style:italic; font-size:9.2pt; line-height:1.2; margin-top:5px;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya...</p>{ttd_asal}</td></tr>
        </table>
