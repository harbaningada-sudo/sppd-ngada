...
<style>
    ...
    /* KHUSUS HALAMAN BELAKANG TURUN LAGI AGAR TIDAK TERPOTONG PRINTER */
    .kertas-belakang { margin-top: 65mm !important; }
    ...
    .inner-table td { border: none !important; padding: 0 !important; font-size: 9.2pt; color: black !important; vertical-align: top; }
...
with st.sidebar:
    ...
    with st.expander("👤 DATA PEGAWAI", expanded=True):
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("➕ Tambah"): st.session_state.jml_p += 1
        with c2:
    ...
    # TOMBOL PRINT - DENGAN DELAY AGAR KONTEN TERISI DULU
    if st.button("🖨️ CETAK SEKARANG"):
        st.components.v1.html("""
            <script>
                setTimeout(function(){ window.parent.print(); }, 1000);
            </script>
        """, height=0)
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        ...
        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{rv("I.", "Berangkat dari", "Bajawa", tgl_berangkat)}<table class="inner-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="3%">:</td><td>{tujuan}</td></tr></table>{ttd_global}</td></tr>
