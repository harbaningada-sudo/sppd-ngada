...
def get_ttd(space): 
    lbl = ttd_label
    jab_final = f"{jab_ttd}," if lbl == "An. BUPATI NGADA" else ""
    ub_final = f"Ub. {ub}," if (ub and lbl == "An. BUPATI NGADA") else ""
    return f'''<div style="margin-left:55%; margin-top:10px; line-height:1.0; text-align:center;"><b>{lbl}</b><br>{jab_final}<br>{ub_final}<div style="height:{space}px;"></div><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''
