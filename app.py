...
<style>
    ...
    /* KHUSUS HALAMAN BELAKANG TURUN LAGI */
    .kertas-belakang { margin-top: 60mm !important; }
...
with st.sidebar:
    ...
    ub_ttd = st.text_input("Ub.", "Asisten Perekonomian dan Pembangunan")
    nip_ttd = st.text_input("NIP", "19710328 199203 1 011")

    # TOMBOL PRINT DENGAN JAVASCRIPT DIRECT
...
ttd_global = f'''<div style="margin-left:55%; margin-top:15px; line-height:1.2; color:black; font-size:10pt;"><table class="visum-table"><tr><td width="40%">Ditetapkan di</td><td width="5%">:</td><td>Bajawa</td></tr><tr><td>Pada Tanggal</td><td width="5%">:</td><td>{tgl_berangkat}</td></tr></table><br><b>An. BUPATI NGADA</b><br>{jab_ttd},<br>{f"Ub. {ub_ttd}," if ub_ttd else ""}<br><br><br><br><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''
...
for p in daftar:
    ...
    if "SPD Belakang" in opsi_cetak:
        def row_v(num, label, val, date_v, is_num=True):
        ...
        html_out += f'''<div class="kertas kertas-belakang"><table class="tabel-border">
            <tr style="height: 180px;"><td width="50%"></td><td style="padding:5px;">{row_v("I.", "Berangkat dari", "Bajawa", tgl_berangkat)}<table class="visum-table"><tr><td width="10%"></td><td width="35%">Ke</td><td width="3%">:</td><td>{tujuan}</td></tr></table>{ttd_global}</td></tr>
            ...
            <tr style="height: 180px;"><td>{row_v("V.", "Tiba Kembali", "Bajawa", tgl_kembali)}</td><td style="padding:5px;"><p style="font-style:italic; font-size:8.8pt; line-height:1.2; margin-top:2px;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya dan semata-mata untuk kepentingan jabatan</p>{ttd_global}</td></tr>
        </table><div style="border:1pt solid black; border-top:none; padding:5px; font-size:10pt;"><b>VI. Catatan Lain-lain</b></div>
        <div style="border:1pt solid black; border-top:none; padding:8px; font-size:8pt; text-align:justify; color:black; line-height:1.1;">
            <b>VII. Perhatian :</b><br>Pejabat yang menerbitkan SPD, pegawai yang melakukan perjalanan dinas, para pejabat yang mengesahkan tanggal berangkat/tiba, serta Bendahara Pengeluaran bertanggung jawab berdasarkan peraturan-peraturan Keuangan Negara apabila negara menderita rugi akibat kesalahan, kelalaian dan kealpaannya.
        </div></div>'''

# 3. REGISTER
if "Register" in opsi_cetak:
    rows = "".join([f"<tr><td class='text-center'>{i+1}</td><td>{p['nama']}</td><td>{no_spt}</td><td>{p['spd']}</td><td>{tgl_berangkat}</td><td>{tujuan}</td></tr>" for i, p in enumerate(daftar)])
    html_out += f'''<div class="kertas"><h3 class="text-center text-bold">REGISTER SURAT PERJALANAN DINAS</h3><br><table class="tabel-border"><thead><tr style="background:#eee;"><th>No</th><th>Nama Pegawai</th><th>No SPT</th><th>No SPD</th><th>Tgl Berangkat</th><th>Tujuan</th></tr></thead><tbody>{rows}</tbody></table></div>'''
