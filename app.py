...
st.set_page_config(page_title="Sistem SPD Ngada - Garansi Logo", layout="wide")
...
<style>
    ...
    /* KOP SURAT MEPET (Line Spacing 1.0) */
    ...
    @media print {
        ...
        table, td { border: 1pt solid black !important; -webkit-print-color-adjust: exact !important; }
        @page { size: legal; margin: 0; }
    }
</style>
""", unsafe_allow_html=True)

# --- CARA ALTERNATIF: LINK URL LOGO ---
# Saya pakai link contoh, silakan ganti dengan link gambar logo Ngada yang sudah di-upload ke internet
LINK_LOGO_PEMDA = "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Lambang_Kabupaten_Ngada.png/150px-Lambang_Kabupaten_Ngada.png"
LINK_LOGO_GARUDA = "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3e/Coat_of_arms_of_Indonesia.svg/150px-Coat_of_arms_of_Indonesia.svg.png"
...
with st.sidebar:
    ...
    with st.expander("📄 DATA SURAT", expanded=True):
        ...
        maksud = st.text_area("Maksud Perjalanan", "Dalam Rangka Monitoring dan Pendataan...")
    ...
    with st.expander("👤 DATA PEGAWAI"):
        ...
        c = st.columns(2)
        if c[0].button("➕"): st.session_state.jml += 1
        if c[1].button("➖") and st.session_state.jml > 1: st.session_state.jml -= 1
        daftar = []
        for i in range(st.session_state.jml):
            ...
            g = st.text_input(f"Gol P-{i+1}", "Penata Muda - III/a", key=f"g{i}")
            j = st.text_input(f"Jabatan P-{i+1}", "Perencana Ahli Pertama", key=f"j{i}")
    ...
    jab_pjb = st.text_input("Pangkat/Gol TTD", "Pembina Utama Muda - IV/c")
    nip_p = st.text_input("NIP TTD", "19710328 199203 1 011")
...
kop_pemda_html = f'<div class="kop-pemda"><img src="{LINK_LOGO_PEMDA}"><div class="kop-teks"><h3>PEMERINTAH KABUPATEN NGADA</h3><h2>SEKRETARIAT DAERAH</h2><p>Jln. Soekarno - Hatta No. 1 Telp (0384) 2225834</p><p class="text-bold">BAJAWA</p></div></div>'
kop_garuda_html = f'<div class="kop-garuda"><img src="{LINK_LOGO_GARUDA}"><h2>BUPATI NGADA</h2></div>'
ttd_global = f'<div style="margin-left:55%; margin-top:15px; line-height:1.2; color:black; font-size:10.5pt;"><table border="0" style="width:100%;"><tr><td>Ditetapkan di</td><td>: Bajawa</td></tr><tr><td>Pada Tanggal</td><td>: {tgl_str(datetime.now())}</td></tr></table><br><b>An. BUPATI NGADA</b><br>Pj. Sekretaris Daerah,<br><br><br><br><br><b><u>{pjb}</u></b><br>{jab_pjb}<br>NIP. {nip_p}</div>'

# --- SPT ---
s_kop = kop_garuda_html if jenis == "Luar Daerah" else kop_pemda_html
rows_peg = "".join([f"<tr><td width='15%'>{'Kepada' if i==0 else ''}</td><td width='5%'>{i+1}.</td><td width='15%'>Nama</td><td width='2%'>:</td><td><b>{p['nama']}</b></td></tr><tr><td></td><td></td><td>Pangkat/Gol</td><td>:</td><td>{p['gol']}</td></tr><tr><td></td><td></td><td>NIP</td><td>:</td><td>{p['nip']}</td></tr><tr><td></td><td></td><td>Jabatan</td><td>:</td><td>{p['jab']}</td></tr>" for i, p in enumerate(daftar)])
html_out += f'<div class="kertas">{s_kop}<div style="margin-top:10px;"><h3 class="text-center text-bold underline" style="margin:0;">SURAT PERINTAH TUGAS</h3><p class="text-center" style="margin:0;">NOMOR : {no_spt}</p></div><table width="100%" style="font-size:10.5pt; margin-top:15px;"><tr><td width="15%">Dasar</td><td width="2%">:</td><td>{anggaran}</td></tr></table><p class="text-center text-bold" style="margin:15px 0;">M E M E R I N T A H K A N</p><table width="100%" style="font-size:10.5pt;">{rows_peg}</table><table width="100%" style="font-size:10.5pt; margin-top:15px;"><tr><td width="15%">Untuk</td><td width="2%">:</td><td>{maksud} ke {tujuan}</td></tr></table>{ttd_global}</div>'
...
for p in daftar:
    ...
    <div style="margin-left:60%; font-size:10pt; line-height:1.0;">
        <table border="0">
            ...
            <tr><td>Kode No</td><td>: {kode_no_spd}</td></tr>
            <tr><td>Nomor</td><td>: {p['spd']}</td></tr>
        </table>
    </div>
    <h3 class="text-center text-bold underline" style="margin:5px 0 0 0; font-size:12pt;">SURAT PERJALANAN DINAS</h3>
    <h3 class="text-center text-bold" style="margin:0 0 10px 0; font-size:12pt;">(SPD)</h3>
    <table class="tabel-border">
        ...
        <tr><td>5.</td><td>Alat angkut yang digunakan</td><td colspan="3">{alat}</td></tr>
        ...
        <tr><td>8.</td><td>Pengikut: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Nama</td><td width="20%" class="text-center">Tanggal Lahir</td><td colspan="2" class="text-center">Keterangan</td></tr>
    ...
    # --- VISUM BELAKANG ---
    ttd_v = f"""<div style="text-align:center; line-height:1.1; font-size:9.5pt; color:black;"><br><b>An. BUPATI NGADA</b><br>Pj. Sekretaris Daerah,<br><br><br><br><b><u>{pjb}</u></b><br>{jab_pjb}<br>NIP. {nip_p}</div>"""
    ...
    <table class="tabel-border">
        ...
        <tr style="height: 150px;"><td>III. &nbsp; Tiba di : <br>&nbsp;&nbsp;&nbsp;&nbsp;Pada Tanggal : </td><td>&nbsp;&nbsp;&nbsp;&nbsp;Berangkat dari : <br>&nbsp;&nbsp;&nbsp;&nbsp;Ke : <br>&nbsp;&nbsp;&nbsp;&nbsp;Pada Tanggal : </td></tr>
        <tr style="height: 150px;"><td>IV. &nbsp; Tiba di : <br>&nbsp;&nbsp;&nbsp;&nbsp;Pada Tanggal : </td><td>&nbsp;&nbsp;&nbsp;&nbsp;Berangkat dari : <br>&nbsp;&nbsp;&nbsp;&nbsp;Ke : <br>&nbsp;&nbsp;&nbsp;&nbsp;Pada Tanggal : </td></tr>
        <tr style="height: 180px;"><td>V. &nbsp; Tiba Kembali : Bajawa<br>&nbsp;&nbsp;&nbsp;&nbsp;Pada Tanggal : </td><td><p style="font-style:italic; font-size:9pt; line-height:1.2;">Telah diperiksa, dengan keterangan bahwa perjalanan tersebut atas perintahnya dan semata-mata untuk kepentingan jabatan</p>{ttd_v}</td></tr>
    </table>
    <div style="border:1pt solid black; border-top:none; padding:8px; font-size:10pt;"><b>VI. Catatan Lain-lain</b></div>
    <div style="border:1pt solid black; border-top:none; padding:8px; font-size:8.5pt; text-align:justify;">
        <b>VII. Perhatian :</b><br>Pejabat yang menerbitkan SPD bertanggung jawab berdasarkan peraturan-peraturan Keuangan Negara apabila negara menderita rugi akibat kesalahan, kelalaian dan kealpaannya.
    </div></div>"""
