...
<style>
    ...
    /* KOP KHUSUS GARUDA (SPT LUAR DAERAH) */
...
with st.sidebar:
    ...
    # --- INPUT BARU: PILIHAN JENIS PERJALANAN ---
    jenis_perjalanan = st.selectbox("Jenis Perjalanan", ["Dalam Daerah", "Luar Daerah"])
    ...
    label_ttd = st.selectbox("Label TTD", ["An. BUPATI NGADA", "BUPATI NGADA", "WAKIL BUPATI NGADA", "Pj. Sekretaris Daerah"])
    ub = st.text_input("Ub. (Kosongkan jika tidak ada)", "Asisten Perekonomian dan Pembangunan")
    nip_ttd = st.text_input("NIP Penandatangan", "19710328 199203 1 011")
...
def get_ttd_statis(space):
    ub_txt = f"Ub. {ub},<br>" if ub else ""
    return f'''<div style="text-align:center; line-height:1.2; font-size:10.5pt;"><b>{label_ttd}</b><br>{ub_txt}<div style="height:{space}px;"></div><b><u>{pjb}</u></b><br>{gol_pjb}<br>NIP. {nip_ttd}</div>'''

html_out = '<div class="main-container">'

if "SPT" in opsi_cetak:
    # SPT LOGIC: Jika Luar Daerah pakai Logo Garuda
    if jenis_perjalanan == "Luar Daerah":
        ...
        kop_spt = f'''<table class="kop-table"><tr><td width="15%"><img src="data:image/png;base64,{logo.PEMDA}" width="75"></td><td class="kop-teks"><h3>PEMERINTAH KABUPATEN NGADA</h3><h2>SEKRETARIAT DAERAH</h2><p>BAJAWA</p></td><td width="15%"></td></tr></table>'''
...
if "SPD Belakang" in opsi_cetak:
    ...
    # LOGIC SPD BELAKANG: Jika Luar Daerah, Poin II Kiri Kosong
    ttd_ii_kiri = "" if jenis_perjalanan == "Luar Daerah" else ttd_bk
    ...
    html_out += f'''<div class="kertas"><table class="tabel-border" style="height:88%;">
        ...
        <tr style="height: 190px;"><td>{rv("II.", "Tiba di", tujuan, tgl_bkt)}<br>{ttd_ii_kiri}</td><td style="padding:10px;">{rv("", "Berangkat dari", tujuan, tgl_kbl, False)}<table class="visum-table"><tr><td width="35%">Ke</td><td width="5%">:</td><td>Bajawa</td></tr></table></td></tr>
    ...
    </table><div style="border:1pt solid black; border-top:none; padding:8px; font-size:10.5pt;"><b>VI. Catatan Lain-lain</b></div><div style="border:1pt solid black; border-top:none; padding:8px; font-size:8.6pt; text-align:justify; color:black; line-height:1.2;"><b>VII. Perhatian :</b> Pejabat yang menerbitkan SPD... bertanggung jawab berdasarkan peraturan Keuangan Negara...</div></div>'''
